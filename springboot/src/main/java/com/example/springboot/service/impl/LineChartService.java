...
public class LineChartService implements ILineChartService {
    ...
    public Object getLineChart(String range) {
        ...
        List<BorrowReturnCountPO> borrowData = borrowMapper.countByCDate(range);
        List<BorrowReturnCountPO> reternData = returnMapper.countByADate(range);
        // insert data
    ...
    private List<Integer> countList(List<BorrowReturnCountPO> poList, List<String> dateRangeStr) { // get some help form others
        ...
        for (String date: dateRangeStr) {
            ...
            Integer count = poList.stream().filter(po -> date.equals(po.getDate())).map(BorrowReturnCountPO::getCount).findFirst().orElse(0);
            intList.add(count);
        }
        return intList;
    }
